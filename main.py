...
def main(in_dirs: list, out: str, dos_path: str):
    ...
    lists_path = os.path.join(out, 'LISTS')
    print(f'Generating Lists {lists_path}')
    os.mkdir(lists_path)
    pcxt_list = os.path.join(lists_path, 'PCXT')
    print(f'Generating PCXT list for Rloader in {pcxt_list}...')
    os.mkdir(pcxt_list)
    os.mkdir(os.path.join(pcxt_list, 'TITLES'))
    os.mkdir(os.path.join(pcxt_list, 'INFO'))
    os.mkdir(os.path.join(pcxt_list, 'IN_PROGRS'))
    list_txt = os.path.join(pcxt_list, 'LIST.TXT')
