...
void Check_Graphics(){
	...
	if (CARD) {//CGA TANDY EGA VGA
		Color_S = 0x3F; Color_N = 0x1F; Color_E = 0x4F; TILE_MAP = (unsigned char *)0xB8000000L;
		for (i =0;i<16;i++)TILE_MAP_ROWS[i] = (unsigned char *)0xB8000000L+((i+3)*160);
	}
	else {//MDA
		Color_S = 0x70; Color_N = 0x10; Color_E = 0x10; TILE_MAP = (unsigned char *)0xB0000000L;
...
int main() {
	...
	programs = load();
    while(!exit) {
		if (restore_menu) {Draw_Menu();restore_menu = 0;}
		if (input){
			for (i = 0; i < 16; i++) {
